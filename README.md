...
В админке: Настройки → Обновить, если на GitHub есть новый коммит. Кабинет пересоберётся, переписка на месте.

Либо снова та же команда установки. Домен и секреты в `deploy/.env` не затираются.
